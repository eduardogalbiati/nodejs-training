...
employerModel.create = async function(data) {
    ...
    const results = await db.query("INSERT INTO employers (name, email) VALUES (?, ?);", [employer.name, employer.email]);
...
employerModel.update = async function(id, data) {
    ...
    const results = await db.query("UPDATE employers SET  name = ?, email = ? WHERE id = ?;", [employer.name, employer.email, id]);
        
    return (await this.getById(id));
};

employerModel.destroy = async function(id) {
    const results = await db.query("UPDATE employers SET active = 0 WHERE id = ?;", [id]);
